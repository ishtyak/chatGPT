...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const currentSessions = await getUserSessions(params.id);
      setUser(currentUser);
  ...
  const subscription = useMemo(
    () =>
      adminMockState.subscriptions.find((item) => item.userId === params.id),
    [params.id],
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description="This removes the user, subscription record, and session history from the admin mock store."
        danger
