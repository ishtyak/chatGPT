...
export default function SettingsPage() {
  ...
  const saveMaintenance = async () => {
    ...
    const message = settings.maintenanceMode ? "Maintenance enabled" : "Maintenance disabled"
    enqueueSnackbar(message, { variant: settings.maintenanceMode ? "warning" : "success" })
  };

  const saveRateLimits = () => {
    enqueueSnackbar("Rate limits saved", { variant: "success" })
  };
          ...
          <div className="mt-4 flex justify-end">
            <button
              onClick={async () => {
                ...
                setSettings(next);
                enqueueSnackbar("Secrets saved", { variant: "success" })
              }}
              className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white"
            >
              Save secrets
          ...
          <div className="mt-4 flex justify-end">
            <button
              onClick={() => saveSettings({
                ...
                enqueueSnackbar("Google OAuth saved", { variant: "success" })
              })}
              className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white"
            >
              Save Google OAuth
          ...
          <div className="mt-4 flex justify-end">
            <button
              onClick={() => saveSettings({ replicateApiToken: settings.replicateApiToken })
                .then(() => {
                  enqueueSnackbar("Replicate API token saved", { variant: "success" })
                })}
              className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white"
            >
