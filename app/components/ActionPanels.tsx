            ...
            <div className="flex gap-2">
              {["Low", "Medium", "High"].map((q) => (
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-xs font-medium transition-all ${quality === q ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
                >
                  {q}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-1.5">
              {EDIT_SIZES.map((s) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-all ${size === s ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
                >
                  {s}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4].map((n) => (
                <button
                  key={n}
                  onClick={() => onCountChange(n)}
                  className={`h-8 w-8 rounded-full border text-sm font-medium transition-all ${count === n ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
                >
                  {n}
                </button>
              ))}
      ...
      <div className="grid grid-cols-2 gap-6 p-4">
          ...
          <div className="flex gap-2">
            {["2x", "4x"].map((s) => (
              <button
                key={s}
                onClick={() => setScale(s)}
                className={`rounded-full border px-5 py-1.5 text-xs font-medium transition-all ${scale === s ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
              >
                {s}
              </button>
            ))}
        ...
        <div>
          ...
          <p className="mt-2 text-[11px] text-zinc-400 leading-relaxed">
            Creativity applies to images only. Higher values add more AI
            enhancement.
          </p>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {MUSIC_TAGS.map((t) => (
              <button
                key={t}
                onClick={() => toggleTag(t)}
                className={`rounded-full border px-3 py-1 text-xs font-medium transition-all ${tags.includes(t) ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
              >
                {t}
              </button>
            ))}
        ...
        <div>
          ...
          <p className="mt-1 text-[11px] text-zinc-400">
            Leave empty for instrumental or let AI write lyrics based on your
            prompt.
          </p>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-64 appearance-none rounded-xl border border-zinc-900 bg-white px-3 py-2 pr-8 text-sm text-zinc-800 outline-none cursor-pointer"
            >
              {LANGUAGES.map((l) => (
          ...
          <p className="mt-1 text-[11px] text-zinc-400">
            Select a language for native pronunciation
          </p>
        </div>
        <div>
          <div className="mb-2 flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 rounded-full bg-zinc-900 p-0.5 ml-2">
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs font-medium transition-all ${tab === "default" ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}
              >
                Default voices
              </button>
              <button
                onClick={() => setTab("custom")}
                className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium transition-all ${tab === "custom" ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}
              >
                <svg
                  width="11"
                  height="11"
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {TTS_VOICES.map((v) => (
              <button
                key={v}
                onClick={() => setVoice(v)}
                className={`flex items-center justify-between rounded-xl border px-2.5 py-2 text-xs font-medium transition-all ${voice === v ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-700 hover:border-zinc-400"}`}
              >
                {v}
                <svg
                  width="12"
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white shadow-lg">
      ...
      <div className="p-4 flex flex-col gap-4">
        <p className="text-xs text-zinc-500">
          Upload an audio file to change its voice. The speech content is
          preserved.
        </p>
        <div>
          ...
          <div className="flex flex-col gap-1.5">
            {["No comments", "Minimal", "Detailed"].map((c) => (
              <button
                ...
                className={`rounded-full border px-4 py-1.5 text-xs font-medium transition-all text-left ${comments === c ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}
              >
                {c}
              </button>
            ))}
